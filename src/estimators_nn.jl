...
export NetworkArg, TrainingArg, prepare_training, create_mlp, generate_samples, train_loop!, test, losses_rmse
...
"""
    TrainingArg(batchsize, lossf, lr, epoch, tv_split, patience )
...
"""
function NetworkArg(model::BiophysicalModel, protocol::Protocol,params::Tuple{Vararg{String}},paralinks::Tuple{Vararg{Pair{String}}},tissuetype::String,sigma::Float64,noise_type::String = "Gaussian",dropoutp=0.2)
    ...
    # adjust "50"
    nsamples = npar*50
    arg = NetworkArg(model,protocol,params,paralinks,tissuetype,sigma,noise_type,hidden_layers,nsamples,nin,nout,dropoutp)
...
mlp is the network model; inputs and labels are arrays of signals and scaled tissue parameters;
gt is a dict containing the ground truth tissue parameters without scaling
"""
function prepare_training(arg::NetworkArg)

    mlp = create_mlp(arg.nin, arg.nout, arg.hidden_layers, arg.dropoutp)

    (inputs, labels, gt) = generate_samples(arg.model, arg.protocol,
        arg.params, arg.nsamples, arg.paralinks, arg.tissuetype,
        arg.sigma, arg.noise_type)
...
function create_mlp(ninput::Int64,noutput::Int64,hiddenlayers::Tuple{Vararg{Int64}},dropoutp::Float64=0.2)
    ...
    mlp = Chain(mlp...,
        Dropout(dropoutp),Dense(hiddenlayers[end]=>noutput))

    return mlp
end

"""
    generate_samples(model, protocol, params, nsamples, paralinks, tissuetype, sigma, noisetype)
generate training samples for a model using uniform coverage of tissue parameters and specified noise model
"""
function generate_samples(model::BiophysicalModel, protocol::Protocol,params::Tuple{Vararg{String}},nsample::Int64,paralinks::Tuple{Vararg{Pair{String}}},tissuetype::String,sigma::Float64,noise_type::String = "Gaussian")
    ...
    for para in params
        if !hasfield(typeof(model),Symbol(para))
            ...
            vecs = scaling[subfieldname][1][1] .+ (scaling[subfieldname][1][2] - scaling[subfieldname][1][1]).*rand(Float64,nsample)
            push!(params_labels,(vecs.*scaling[subfieldname][2].*scaling[subfieldname][3])')
        ...
        elseif para == "fracs"
            ...
            vecs = scaling[fieldname][1][1] .+ (scaling[fieldname][1][2] - scalings[fieldname][1][1]).*rand(Float64,nsample)
    ...
    elseif noise_type == "Rician"
        ...
        signals .= sqrt.((signals .+ rand(Normal(0,sigma),nvol,nsample)).^2.0 .+ rand(Normal(0,sigma),nvol,nsample).^2.0)
    end
...
function train_loop!(mlp::Chain{T},arg::TrainingArg,inputs::Array{Float64,2},labels::Array{Float64,2}) where T
    ...
    val_set = Flux.DataLoader((@views inputs[:,1:tv_index],@views labels[:,1:tv_index]),batchsize=arg.batchsize)
    train_set = Flux.DataLoader((@views inputs[:,tv_index+1:end],@views labels[:,tv_index+1:end]),batchsize=arg.batchsize)
